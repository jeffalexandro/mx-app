...
function init_entrega_coleta() {
	...
	$$.getJSON (
		...
		$$.each(data.awb_lista, function (index, value_j) {
			...
			if (value_j.status == 0) {
				...
				var btn_init = '<button class="button button-fill button-raised color-green iniciar_coleta_entrega" id="iniciar_coleta_entrega_'+value_j.rota_log_id+'"/>Iniciar</button>\
								<button class="button button-fill button-raised color-red cod_disp_coleta_entrega" id="cod_disp_coleta_entrega_'+value_j.rota_log_id+'"/>Cód Disp</button>';
			}
			else if(value_j.status == 1){
				var card_color = "card_green";
				var btn_init = '<button class="button button-fill button-raised pod_coleta_entrega" id="pod_coleta_entrega_'+value_j.rota_log_id+'"/>POD</button>\
								<button class="button button-fill button-raised color-red cod_disp_coleta_entrega" id="cod_disp_coleta_entrega_'+value_j.rota_log_id+'"/>Cód Disp</button>';	
			}
		...
		$$('.pod_coleta_entrega').on('click', function (e) {
		    ...
		    myApp.confirm(form, 'POD', function () {
		        $$.getJSON (
					'http://messenger.com.br/app/controller.php',
					...
					function (data) {
						if(data.sucesso){
							myApp.alert('', 'POD informado com sucesso', function () { mainView.router.refreshPage(); });	
						}
						else{
		...
		$$('.cod_disp_coleta_entrega').on('click', function (e) {	
							...
							<ul>\
								<li class="item-content">\
      								<div class="item-inner">\
      									<div class="item-input">\
      										<select>\
												<input type="text" name="cod_disp_entrega_coleta_descricao" id="cod_disp_entrega_coleta_descricao" placeholder="Informe um código de disposição" / >\
								            </select>\
      									</div>\
      								</div>\
      							</li>\
			...
			myApp.confirm(form, 'POD', function () {
		        $$.getJSON (
					...
					function (data) {
						if(data.sucesso){
							myApp.alert('', 'Código de disposição informado com sucesso', function () { mainView.router.refreshPage(); });	
						}
						else{
							myApp.alert('', 'Ocorreu um erro ao salvar informações, tente novamente.');	
						}
