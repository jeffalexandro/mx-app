...
function _init_tarefas(data) {
	...
	$$('.iniciar_coleta_entrega').on('click', function (e) {						    
		...
		// if(navigator.connection.type == "none") {
			...
			$$.each(fileObj.awb_lista, function (index, value_j) {
			...
			myApp.alert('Início de Coleta/entrega armazenada para atualização posterior', 'Você esta offline', function () { mainView.router.refreshPage(); });
	...
	$$('.pod_coleta_entrega').on('click', function (e) {
	    ...
	    myApp.prompt(form, 'Confirmação de Entrega/Coleta', function () {
	    	...
	    	// if(navigator.connection.type == "none") {
				...
				$$.each(fileObj.awb_lista, function (index, value_j) {
					...
					// if(value_j.)
					if(index == ordem_id) {
						fileObj.awb_lista[index].atualizacao.push({"Data" : new Date().toLocaleString(), "Descrição" : "POD" });
						fileObj.awb_lista[index].status[0] = 2;							
					}
				...
				myApp.alert('Confirmação de Entrega/Coleta armazenada para atualização posterior', 'Você esta offline', function () { mainView.router.refreshPage(); });	
			// }
			// else {
		 //        $$.getJSON (
			// 		'http://messenger.com.br/app/controller.php',
	...
	$$('.cod_disp_coleta_entrega').on('click', function (e) {	
		...
		var form = '<div class="list-block">\
						<ul>\
							<li class="item-content" data-for="email_login">\
							<div class="item-inner">\
								<div class="item-input">\
									<select>\
										<option hidden >Selecione um código</option>';

										$$.each(sgi_codigo_disposicao, function (index, value) {

											form += '<option value="'+value.id+'">'+value.descricao+'</option>';
		...
		myApp.prompt(form, 'Código de Pendência', function () {
			...
			// if(navigator.connection.type == "none") {				
				...
				$$.each(fileObj.awb_lista, function (index, value_j) {
					...
					if(index == ordem_id) {
						if (typeof fileObj.awb_lista[index].atualizacao == 'undefined') {
						  	
						  	fileObj.awb_lista[index].atualizacao = {};

						}
						
						fileObj.awb_lista[index].atualizacao.push({"Data" : new Date().toLocaleString(), "Descrição" : "Cód de Disp" });
						fileObj.awb_lista[index].status[0] = 9;
					}
				...
				myApp.alert('Código de Pendência Armazenado para atualização posterior', 'Você esta offline', function () { 					
